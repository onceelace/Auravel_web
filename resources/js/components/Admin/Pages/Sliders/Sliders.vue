              ...
              <div class="card-body table-responsive" style="min-height: 400px;">
                ...
                <div class="row mb-2">
                    <div class="col-md-6 col-xs-12">
                        <a class="btn btn-light" style="border-radius: 0;" href="/admin/pages/sliders/new">
                            Add New <i class="fas fa-user-plus fa-w"></i>
                        </a>
                        <a class="btn btn-light" style="border-radius: 0;" @click="loadSliders()">
                ...
                <b-table sticky-header striped hover show-empty
                    ...
                    <template v-slot:cell(actions)="row">
                        <b-button size="sm" @click="editSlider(row.item)" class="mr-2" variant="link">
...
<script>
    export default {
        ...
        methods: {
            loadSliders() {
                this.filter = null;
                axios.get("/api/admin/pages/sliders").then(({ data }) => (this.items = data));
            },
            editSlider(info) {
                this.$router.push({ path : '/admin/pages/sliders/edit/' + info.id });
            },
            deleteRecord(id) {
                Swal.fire({
                    title: 'Are you sure?',
                    ...
                    .then((result) => {
                        if(result.value)
                        {
                            ...
                            axios.delete("/api/admin/pages/sliders/" + id)
                            .then((result) => {
                                console.log(result);
                                if (result.data.message === 'Deleted') {
                                     Toast.fire({
                                        type: 'success',
                                        title: 'Page Slider has been deleted'
                                    })
                                    this.loadSliders();
                                }
                            })
                            .catch(() => {
                                if (result.value) {
                                    Swal.fire(
                                    'Failed!',
                                    'Failed to delete the Page Slider.',
                                    'success'
                                    )
                                }
                            });
